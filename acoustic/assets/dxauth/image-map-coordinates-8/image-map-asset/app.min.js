      ...
      , d = {
        defaults: {
            ...
            input: {
                ...
                color:{
                    selector: 'input[name^="im["][name$="][color]"]',
                    fn: function() {
                        return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                    }
                ...
                onhovercolor:{
                    selector: 'input[name^="im["][name$="][onhovercolor]"]',
                    fn: function() {
                        return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                    }
                ...
                onclickcolor:{
                    selector: 'input[name^="im["][name$="][onclickcolor]"]',
                    fn: function() {
                        return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                    }
                ...
                opacity:{
                    selector: 'input[name^="im["][name$="][opacity]"]',
                    fn: function() {
                        return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                    }
        ...
        init: function(e, f) {
            ...
            if (!e) {
                ...
                console.log("reading value from html");
                var url = "";
                var savedResult = document.getElementById("result-saved-from-acoustic").value;
                if(savedResult.length > 0)
                {
                    var imageEleString = savedResult?.split("</map>")?.length > 0 ? savedResult?.split("</map>")[1] : "";
                    var imageEle = new DOMParser().parseFromString(imageEleString, "text/xml");
                    var imageId = imageEle.getElementsByTagName("img")[0].getAttribute("id");
                    var imageName = imageEle.getElementsByTagName("img")[0].getAttribute("name");                   

                    if(imageId.length > 0 && imageName.length > 0)
                    {                      
                       document.getElementById("image-id").value = imageId;
                       document.getElementById("image-name").value = imageName;
                       var parentObj = window.parent;
                      // var hubId = "70aea51c-4e79-4d9e-bbf5-4a2c21a23a3c";
                      var hubId = "a590dc33-59b3-4b9b-b542-612ac16a7b39";
                       ...
                       if(parentObj!=null){
                        ...
                        url = "https://content-eu-1.content-cms.com/"+hubId+"/dxdam/"+imageId.substring(0,2) +"/"+imageId+"/"+imageName;
                        console.log(url);
                    }
                }
        ...
        bindEvents: function(b) {
            ...
            a("#save-initial-response-ready").change(function()
            {
                var x = a("#image-map").imageMapper("asHTML");                
                a("#save-initial-response").val(x);
                a("#save-initial-response").trigger("change");
            }),
            f.on("click", function(a) {     
                var f = d.getPosition.apply(e, [b, a]);

                console.log("f position", f);
            ...
            f.on("mousemove touchmove", function(f) {
                ...
                a.each(c[b].area[m].coords, function(a, b) {
                    console.log("natural x on click",b.naturalX);
                    console.log("natural y on click",b.naturalY);

                    console.log("k natural x on click",k.naturalX);
                    console.log("k natural y on click",k.naturalY);

                    console.log("h natural x on click",h.naturalX);
                    console.log("h natural y on click",h.naturalY);

                    console.log("l ratio x on click",l.ratioX);
                    console.log("l ratio y on click",l.ratioY);
                    ...
                    console.log("c natural x on click",c.naturalX);
                    console.log("c natural y on click",c.naturalY);
                    
                    //condition setting new natural coordinates
                    console.log("natural width",n[0].naturalWidth);


                    c.naturalX < 0 || c.naturalX >= n[0].naturalWidth ? p = !0 :
                     (c.naturalY < 0 || c.naturalY >= n[0].naturalHeight) && (p = !0),
                    o[a] = c
                }),
        ...
        getClientPosition: function(b, c) {
            ...
            var customClientX =Math.round(c.naturalX / f.ratioX) ;
            var customClientY =Math.round(c.naturalY / f.ratioY);
        
            console.log("custom client coord", customClientX, " ",customClientY);
        ...
        asHTML: function(b) {
            ...
            a.each(c[b].area, function(d, e) {
                ...
                if (a.each(e.coords, function(a, b) {
                    console.log("natural x is ", b.naturalX, " natural y is", b.naturalY);
                    console.log("original x is ", b.originalX, " original y is", b.originalY);
                    
                    //var originOffsetX = $("#x-origin-offset").val;
                   //var originOffsetY = $("#y-origin-offset").val;


                    //f.push(b.originalX, b.originalY)
                ...
                console.log("original val", c[b].area[d].originalX );
                var i = a("<area />");
                i.attr("target", c[b].area[d].target),
                i.attr("alt", c[b].area[d].title),
                i.attr("title", c[b].area[d].title),
                i.attr("opacity", c[b].area[d].opacity),
                i.attr("color", c[b].area[d].color),
                i.attr("onclickcolor", c[b].area[d].onclickcolor),
                i.attr("onhovercolor", c[b].area[d].onhovercolor),
            ...
            f.append("\n    " + g.join("\n    ") + "\n");
            var h = "";
            var imageId = document.getElementById("image-id").value.trim();
           ...
           // return h + "\n" + d[0].outerHTML + "\n\n" + f[0].outerHTML
           return h + "\n" + "\n\n" + f[0].outerHTML + image[0].outerHTML
        }  
    };
    ...
    a(document).on("init", function() {
        ...
        a(document).on("click", "#image-mapper-upload", function() {
            ...
            if(imageid.length > 0 && imageName.length > 0 )
            {
                ...
                var url = "https://content-eu-1.content-cms.com/"+hubId+"/dxdam/"+imageid.substring(0,2) +"/"+imageid+"/"+imageName;
                console.log(url);
